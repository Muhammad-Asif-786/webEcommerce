      ...
      <div className='bg-white w-full p-4 max-w-2xl mx-auto rounded overflow-y-auto h-full max-h-[95vh]'>
        ...
        <form className='grid gap-4 p-3' onSubmit={handleSubmit}>
          ...
          <div className='grid gap-1'>
            ...
            <textarea id='description' name='description' value={data.description} onChange={handleChange} required rows={3}
          ...
          <div>
            ...
            <label htmlFor='productImage' className='bg-blue-50 h-24 border rounded flex justify-center items-center cursor-pointer'>
              ...
              <input type='file' id='productImage' className='hidden' accept='image/*' onChange={handleUploadImage} multiple />
            </label>

            <div className='flex flex-wrap gap-4 mt-2'>
              {data.image.map((img, index) => (
                <div key={img+index} className='h-20 w-20 bg-blue-50 border relative group'>
                  <img src={img} alt={img} className='w-full h-full object-scale-down cursor-pointer'
          ...
          <div className='grid gap-1'>
            ...
            <select className='bg-blue-50 border w-full p-2 rounded' value={selectCategory} onChange={(e)=>{
              ...
              if(exists){ toast.error("Category already added"); setSelectCategory(""); return }
            ...
            <div className='flex flex-wrap gap-3 mt-2'>
              {data.category.map((c,index)=>(
                <div key={c._id+index} className='flex items-center gap-1 bg-blue-50 text-sm p-1 rounded'>
                  <p>{c.name}</p>
                  <div onClick={()=>handleRemoveCategory(index)} className='cursor-pointer hover:text-red-500'><IoClose size={18}/></div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className='grid gap-1'>
            ...
            <select className='bg-blue-50 border w-full p-2 rounded' value={selectSubCategory} onChange={(e)=>{
              ...
              const exists = data.subCategory.some(c => c._id === subCategory._id)
              if(exists){ toast.error("SubCategory already added"); setSelectSubCategory(""); return }
            ...
            <div className='flex flex-wrap gap-3 mt-2'>
              {data.subCategory.map((c,index)=>(
                <div key={c._id+index} className='flex items-center gap-1 bg-blue-50 text-sm p-1 rounded'>
                  <p>{c.name}</p>
                  <div onClick={()=>handleRemoveSubCategory(index)} className='cursor-pointer hover:text-red-500'><IoClose size={18}/></div>
                </div>
              ))}
            </div>
          </div>
          ...
          {['unit','stock','price','discount'].map((field)=>
            <div key={field} className='grid gap-1'>
              ...
              <input id={field} type={field==='stock' || field==='price' || field==='discount' ? 'number':'text'} 
                name={field} value={data[field]} onChange={handleChange} required
                className='bg-blue-50 p-2 outline-none border focus-within:border-primary-200 rounded' />
            </div>
          )}
          ...
          <button type="submit" className='bg-green-500 hover:bg-green-600 py-2 rounded font-semibold'>
            Update Product
          </button>
        </form>
...
//               <button
//               type="submit"
//                 className='bg-green-500 hover:bg-green-600 py-2 rounded font-semibold'
//               >
//                 Update Product
//               </button>
//             </form>
